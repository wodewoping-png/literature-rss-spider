...
# 例："contentInfo":{"authors":["A","B","C"]}
NATURE_CONTENTINFO_AUTHORS_RE = re.compile(
    r'"contentInfo"\s*:\s*\{\s*"authors"\s*:\s*(\[[^\]]*\])',
    flags=re.IGNORECASE | re.DOTALL
)
...
# ✅ creator list 统一取最后一个，并返回 trace
...
# ✅ 新增：从 Nature 页面源码里抓 contentInfo.authors 的最后一位
def extract_nature_last_author_from_page_source(page) -> str:
    try:
    ...
    m = NATURE_CONTENTINFO_AUTHORS_RE.search(html)
    if not m:
        return ""

    arr_str = m.group(1)
    try:
        authors = json.loads(arr_str)
        if isinstance(authors, list) and authors:
            last = authors[-1]
...
def enrich_with_html_then_api(records: list[dict]):
    # ✅ 改动：Nature 若作者为空，也进入 HTML 阶段（即使已有摘要）
    need_html = any(
        (
            ("nature.com" in (r.get("link","").lower()) and not (r.get("last_author") or "").strip())
            or
    ...
    if need_html:
        print("\n🔧 HTML阶段：抓 Nature/RSC（headless适配）...（Nature作者为空也会尝试从页面源码抓最后作者）")
        with sync_playwright() as p:
            ...
            for r in records:
                ...
                need_this = False
                if is_nature and not (r.get("last_author") or "").strip():
                    need_this = True
                if (is_nature or is_rsc) and not (r.get("abstract") or "").strip():
                    need_this = True

                if not need_this:
                    continue

                abs_txt, abs_src, nature_la = get_html_abstract_and_nature_author(page, r.get("link", ""))

                if abs_txt and not (r.get("abstract") or "").strip():
                    r["abstract"] = abs_txt.strip()
                    r["abstract_source"] = abs_src

                # ✅ 新增规则：Nature 作者为空 -> 用页面源码 contentInfo.authors[-1]
                if is_nature and not (r.get("last_author") or "").strip() and nature_la:
                    r["last_author"] = nature_la.strip()
                    r["last_author_source"] = "nature_html"
                    _dbg_author(f" [AUTHOR] html ✅ {r['last_author']} via nature_html(contentInfo.authors[-1])")
