...
def _gemini_generate_content(client, messages):
    import requests
...
def _call_with_retries(client, messages, label: str):
    ...
    for attempt in range(1, GOOGLE_AI_MAX_RETRIES + 1):
        try:
            return _gemini_generate_content(client, messages)
        except Exception as e:
            last_err = e
            wait = min(10, 2 * attempt)
            print(f"[gemini:{label}] error on attempt {attempt}: {e} (wait {wait}s)", flush=True)
            time.sleep(wait)
...
def main():
    ...
    print(
        f"[classify] model={GOOGLE_AI_MODEL} batch={CLASSIFY_BATCH_SIZE} text_max_chars={CLASSIFY_TEXT_MAX_CHARS}",
        flush=True,
    )
